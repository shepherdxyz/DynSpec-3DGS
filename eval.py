...
import torch
from scene import Scene, DeformModel, Cube_DeformModel_Normal
import os, time
import numpy as np
...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, save_ims, deform, env_deform):
    if save_ims:
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        env_time_input = fid.unsqueeze(0).expand(view.HWK[0] * view.HWK[1], -1)
        d_xyz, d_rotation, d_scaling = deform.step(xyz.detach(), time_input)
        rendering = render_dynamiccube_normal(view, gaussians, pipeline, background, d_xyz, d_rotation, d_scaling, env_deform, env_time_input,deform_envmap = False)
    ...
    print('iteration:{},psnr:{},ssim:{},lpips:{},fps:{}'.format(iteration, psnr_v, ssim_v, lpip_v, fps))
    dump_path = os.path.join(model_path, 'metric.txt')
    with open(dump_path, 'a') as f:
        f.write('\niteration:{},psnr:{},ssim:{},lpips:{},fps:{}'.format(iteration, psnr_v, ssim_v, lpip_v, fps))
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, save_ims : bool):
    with torch.no_grad():
        ...
        env_deform.load_weights(dataset.model_path, iteration)

        bg_color = [1,1,1] if dataset.white_background else [0, 0, 0]
        background = torch.tensor(bg_color, dtype=torch.float32, device="cuda")

        render_set(dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, background, save_ims, deform, env_deform)
